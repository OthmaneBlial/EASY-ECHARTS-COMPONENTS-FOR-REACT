...
class EchartsLine extends Component {
    ...
    this.state = {
      ...
      option : {
          ...
          xAxis : [
              {
                  type : 'category',
                  boundaryGap : ((this.props.seriesType === 'bar' || this.props.seriesType === 'barStack') ? true : false),
   ...
   switch (this.props.seriesType) {
    case 'areaStack':
      return this.props.seriesNames.map((seriesName, index) =>
               ...
               Object.assign({name: seriesName, type: 'line', stack: 'stackConnector', areaStyle: {normal: {}}, data: seriesData[index]})
            )
      break;
    case 'area':
      return this.props.seriesNames.map((seriesName, index) =>

               Object.assign({name: seriesName, type: 'line', areaStyle: {normal: {}}, data: seriesData[index]})
            )
      break;
      case 'line':
        return this.props.seriesNames.map((seriesName, index) =>

               Object.assign({name: seriesName, type: 'line', data: seriesData[index]})
            )
        break;
      case 'lineStack':
        return this.props.seriesNames.map((seriesName, index) =>

               Object.assign({name: seriesName, type: 'line', stack: 'stackConnector', data: seriesData[index]})
            )
        break;
      case 'bar':
        return this.props.seriesNames.map((seriesName, index) =>

               Object.assign({name: seriesName, type: 'bar', data: seriesData[index]})
            )
        break;
      case 'barStack':
        return this.props.seriesNames.map((seriesName, index) =>

               Object.assign({name: seriesName, type: 'bar', stack: 'stackConnector', data: seriesData[index]})
            )
        break;
  ...
  render() {
    ...
    return (
      <div  ref={chart => this.chart = chart} style={this.props.style} className={this.props.className} >
      </div>
    );
  }
}
